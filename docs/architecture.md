...
- 可选的拼车用户控制面 `/api/portal/*` 与 `/portal`（合同见 [Portal](portal.md)）；
...
| `gateway-portal` | 拼车用户、套餐、订阅与用户 Key/用量用例；不依赖 gateway-admin |
...
Portal 概览通过自身 Usage 端口查询；Store 复用观测查询实现，并按请求冻结的用户归属过滤，共享观测连接预算。成功汇总与健康计数分别使用完整交付和全部请求口径。API 只复用 Admin 已有的纯健康计算和展示转换，不调用管理员服务；`gateway-portal` 不依赖 `gateway-admin`。前端 Key/Portal 页面共用纯用量展示组件，认证与取数保持分域。
...
- OpenAI 默认是透明边界；显式配置 `openai.wire_profile.location` 时，由 Provider 覆盖带环境标记的日期/时区及 Web Search 位置，未配置时保留客户端原值。Responses 请求保留未知字段和字段顺序；SSE、WebSocket、Images 与 standalone
  Search 的业务正文按原始字节转发。canonical facts 从同一数据旁路提取，只用于路由、观测和计费。
- Responses 的业务扩展头保留原始多值字节；`gateway-protocol` 分别定义传输/反代头和下游专属头
  的分类。API 先提取会话语义，再过滤向上游投影的副本；OpenAI Provider 编码与最终 HTTP/WS
  组装同样过滤下游 SDK/浏览器环境头，不能只依赖某个 HTTP 入口或客户端 UA。
  下游链路元数据和压缩协商不跨越该边界，上游认证、请求画像与传输字段仍由 Provider 生成。
...
- 请求画像以配置为启动基线。OpenAI Desktop 与 xAI CLI 的官方版本检查只更新各自负责的运行时画像，
  不回写 `config.yaml`。OpenAI 模型请求目标是 Desktop 内嵌 Core/app-server，而不是独立 CLI 的
  默认身份或 Electron 网络栈。制品版本核实、bundled Core 本地捕获和完整 GUI App 实测是不同证据，
  不能互相替代；版本跟随也不等于重新验证 TLS 或整套 App 行为。
...
- 金额优先使用 Provider 上报的 USD，否则按现有模型价格估算；订阅账号的估算费用不代表上游订阅账单。
  OpenAI 的 `gpt-6-astra` 标准 Token 单价按项目原价的 1.8 倍估算，包含缓存，
  长上下文沿用对应档位普通单价、不额外加价；独立工具调用费不变。
  Terra 与 Luna 按项目要求不采用临时优惠：标准输入/输出分别为 2.5/15 与 1/6 USD / 百万 Token，
  缓存读取为输入价的 10%，缓存写入为输入价的 125%；Flex 为标准价的 0.5 倍，
  长上下文（输入超过 272,000 Token）输入及缓存为普通档的 2 倍、输出为 1.5 倍。
  `gpt-6-astra` 与 `gpt-5.6-sol/terra/luna` 的 Fast/priority Token 费用按对应标准档的 2.5 倍计算，
  对齐 sub2api 的默认 Fast 结算规则；Flex 仍为 0.5 倍，不引入其渠道倍率或免费 Fast 配置。
  这些单价不是官方 API 原价，调价不会重算已结算的历史费用。
