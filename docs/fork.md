...
- Responses 不向上游透传下游 SDK/浏览器环境头，并将 `session_id` 请求头按既有会话语义输出为 `session-id`；未知业务字段和提示词不做品牌清洗。字段范围见 [API 合同](api.md#3-openai-数据面与模型目录)，Pi 自定义提示词是[客户端显式选择](../deploy/README.md#pi-接入与提示词边界)，不提供客户端不可识别保证。
