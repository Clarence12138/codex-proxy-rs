...
#[tokio::test]
async fn unified_login_uses_forwarded_ip_only_for_configured_trusted_peers() {
    for trusted in [false, true] {
        let fixture = crate::support::key_fixture().await;
        let peer = "203.0.113.10".parse::<std::net::IpAddr>().unwrap();
        let app = crate::openai::api_router_with_admin_and_proxies(
            fixture.services,
            if trusted { vec![peer] } else { Vec::new() },
        );
        ...
        request
            .headers_mut()
            .insert("x-forwarded-for", "198.51.100.22".parse().unwrap());
        ...
        assert_eq!(*fixture.auth.login_sources.lock().unwrap(), vec![expected]);
    }
}
