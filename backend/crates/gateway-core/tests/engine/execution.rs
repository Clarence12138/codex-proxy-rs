...
#[test]
fn expired_subscription_key_can_only_verify_identity_not_authorize_inference() {
    use gateway_core::policy::{ClientOwnerScope, OwnerScopeId};
    let owner = ClientOwnerScope::new(
        OwnerScopeId::new("expired-owner").unwrap(),
        RateLimits::unlimited(),
        true,
        None,
    ...
    let service = DefaultExecutionService::new(
        RuntimeSnapshotHandle::new(client_snapshot_with_owner(Some(owner))),
        Arc::new(TrackingExecutionStore::default()),
        ProviderRegistry::default(),
        Arc::new(UnusedAdmissions),
        Arc::new(UnusedCircuits),
    ...
    assert!(service.verify_client_key("sk_usage_test").is_ok());
    assert!(service.authenticate("sk_usage_test").is_err());
    assert!(service.verify_client_key("wrong-key").is_err());
}
...
fn client_snapshot() -> RuntimeSnapshot {
    client_snapshot_with_owner(None)
}

fn client_snapshot_with_owner(
    owner: Option<gateway_core::policy::ClientOwnerScope>,
) -> RuntimeSnapshot {
    let provider = ProviderKind::new("openai").expect("provider kind");
    ...
    if let Some(owner) = owner {
        policy = policy.with_owner_scope(owner);
    }
    RuntimeSnapshot::new(
        ConfigRevision::new(1).expect("config revision"),
        AccountSelectionPolicy::new(
...
impl ClientAdmissionPort for BoundedAdmissions {
    fn abandon(
        ...
        _: &ClientApiKeyId,
        id: &ModelRequestId,
        _owner_scope_id: Option<&gateway_core::policy::OwnerScopeId>,
    ) {
