...
const realmOptions = [
  { label: '账号登录', value: 'admin', icon: ShieldCheck },
  { label: 'API Key 登录', value: 'key', icon: KeyRound },
]

const isKeyRealm = computed(() => realm.value === 'key')
const realmCaption = computed(() => isKeyRealm.value ? 'API KEY' : 'ACCOUNT LOGIN')
  ...
  >
    ...
    <section class="grid min-h-18 min-w-0 content-start gap-4" aria-labelledby="login-title">
      ...
      <p class="m-0 -ml-2 text-sm leading-[1.45] font-normal text-(--cp-login-description-color)">
        {{ isKeyRealm ? '使用 API Key 查看这把密钥的用量' : '管理员与拼车用户使用账号、密码登录' }}
      </p>
    </section>
    ...
    <div class="grid gap-3">
      <template v-if="!isKeyRealm">
        <div class="grid min-w-0 gap-2">
          <span id="admin-username-label" class="text-cp leading-[1.1] font-bold text-(--cp-login-label-color)">账号</span>
          <BaseInput
            v-model="username"
            name="username"
            aria-label="账号"
